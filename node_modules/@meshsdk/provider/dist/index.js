...
import { parseAssetUnit as parseAssetUnit2 } from "@meshsdk/common";
import {
  deserializeTx,
  fromTxUnspentOutput,
  toTxUnspentOutput,
...
var parseValueForBlockfrost = (utxo) => {
  ...
  utxo.output.amount.forEach(({ unit, quantity }) => {
    if (unit === "lovelace") {
      ...
      const { policyId, assetName } = parseAssetUnit2(unit);
      if (!value[policyId]) {
        value[policyId] = {};
      }
...
var BlockfrostProvider = class {
   ...
   */
  async fetchAddressTxs(address, option = DEFAULT_FETCHER_OPTIONS) {
    const txs = [];
    try {
  ...
  async fetchHandleAddress(handle) {
    ...
    try {
      const assetName = fromUTF83(handle.replace("$", ""));
      const { data, status } = await this._axiosInstance.get(
        `assets/${SUPPORTED_HANDLES2[1]}${assetName}/addresses`
      );
...
import {
  ...
  DEFAULT_FETCHER_OPTIONS as DEFAULT_FETCHER_OPTIONS2,
...
var KoiosProvider = class {
  ...
  constructor(...args) {
    if (typeof args[0] === "string" && args[0].startsWith("http")) {
      this._axiosInstance = axios4.create({
        baseURL: args[0],
        headers: {
      ...
      this._axiosInstance = axios4.create(config);
    }
  }
  /**
   * Obtain information about a specific stake account.
   ...
   * @param address
   */
  async fetchAddressTxs(address, option = DEFAULT_FETCHER_OPTIONS2) {
    try {
  ...
  async fetchHandle(handle) {
    ...
    try {
      ...
      const asset = await this.fetchAssetMetadata(
        `${SUPPORTED_HANDLES3[1]}000de140${assetName}`
      );
      return asset;
    } catch (error) {
      throw parseHttpError(error);
  ...
  async fetchHandleAddress(handle) {
    ...
    try {
      const assetName = fromUTF84(handle.replace("$", ""));
      const { data, status } = await this._axiosInstance.get(
        `asset_addresses?_asset_policy=${SUPPORTED_HANDLES3[1]}&_asset_name=${assetName}`
      );
  ...
  resolveScriptRef = (kScriptRef) => {
    if (kScriptRef) {
      ...
      if (kScriptRef.type.startsWith("plutus")) {
        const normalized = normalizePlutusScript2(
          kScriptRef.bytes,
          "DoubleCBOR"
        );
        script = {
      ...
      if (status === 200 || status === 202) {
        ...
        return data.result.map((val) => {
          ...
          return {
            ...
            tag: val.validator.purpose.toUpperCase() === "PUBLISH" ? "CERT" : val.validator.purpose.toUpperCase(),
            budget: {
...
var MaestroProvider = class {
  ...
  async fetchHandle(handle) {
    ...
    try {
      ...
      const asset = await this.fetchAssetMetadata(
        `${SUPPORTED_HANDLES4[1]}000643b0${assetName}`
      );
  ...
  async submitTx(tx) {
    try {
      ...
      const { data, status } = await this._axiosInstance.post(
        this.submitUrl,
        this.submitTxToBytes ? toBytes3(tx) : tx,
        { headers }
      );
      if (status === 202) return data;
      throw parseHttpError(data);
...
var YaciProvider = class {
  ...
  constructor(baseUrl = "https://yaci-node.meshjs.dev/api/v1/", adminUrl) {
    ...
    if (adminUrl) {
      this._adminAxiosInstance = axios6.create({
        baseURL: adminUrl
      });
    }
  }
  ...
  async fetchHandleAddress(handle) {
    try {
      ...
      const { data, status } = await this._axiosInstance.get(
        `assets/${SUPPORTED_HANDLES5[1]}${assetName}/addresses`
      );
